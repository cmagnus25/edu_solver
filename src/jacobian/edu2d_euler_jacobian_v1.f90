...
!*
!* (08-21-2014: Equation below has been corrected. V/dt term is not needed in the off-diag blocks.
!*              This is the only change made form jacobian_v0.f90 to jacobian_v1.f90.)
!*
!*       [      dRj/dU1]*dU1 + [dRj/dU2]*dU2 + [dRj/dU3]*dU3 +
!*       [V/dtj+dRj/dUj]*dUj + [dRj/dU4]*dU4 + [dRj/dU5]*dU5 = - Rj
!*
...
!*        2.  4x4 Off-Diagonal Block:  jac(j)%off(:,:,k) = [      dRj/dUk], k = 1,2,3,4,5
!*
...
!********************************************************************************
 ...
 use edu2d_constants   , only : p2                ! Double precision
...
!*       See "I do like CFD, VOL.1, 2nd Edition" about how a tangent vector are eliminated.
!*
...
!*
!* Note: The Jacobian is EXACT. No approximations have been made.
!*
!* ------------------------------------------------------------------------------
...
! Note: See "I do like CFD, VOL.1, 2nd Edition" about how a tangent vector is eliminated.
      du = uR - uL
      dv = vR - vL
  R(1,4) = zero
  R(2,4) = du - dqn*nx
...
!
!     (See "I do like CFD, VOL.1, 2nd Edition" for the analytical Jacobian, dFn(u)/du)
...
! 1.2 Compute various deriavives that will be used in the following steps.
!
! See "I do like CFD, VOL.1, 2nd Edition", Section 3.6.5 for the derivation
! of the derivatives of the eigenvalues, speed of sound, Roe-averages, etc.
...
!--------------------------------------------------------------------------------
 ...
 end module edu2d_euler_jacobian
